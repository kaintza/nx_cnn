...
#       |> Axon.conv(32, kernel_size: {3, 3}, activation: :relu)
#       # |> Axon.batch_norm()
#       |> Axon.max_pool(kernel_size: {2, 2}, strides: 0)
#       |> Axon.conv(64, kernel_size: {3, 3}, activation: :relu)
#       # |> Axon.spatial_dropout()
#       # |> Axon.batch_norm()
#       |> Axon.max_pool(kernel_size: {2, 2}, strides: 0)
...
#       |> Axon.dense(10, activation: :softmax)
    ...
    #   model |> Axon.Training.step(:categorical_cross_entropy, Axon.Optimizers.sgd(0.01), metrics: [:accuracy]) |> Axon.Training.train(train_images, train_labels, epochs: 1, compiler: EXLA)
    # |> Nx.backend_transfer()
    # |> IO.inspect()
...
#   prediction =
#     model |> Axon.predict(final_training_state[:params], test_images)
#     |> Nx.argmax(axis: -1)
